...
def remove_outer_link(string):
    # 外部リンクを除去
    dst = re.sub(r'https?://[\w!?/\+\-_~=;\.,*&@#$%\(\)\'\[\]]+','',string)
    return dst

def remove_htmltag(string):
    #htmlタグを除去
    dst = re.sub(r'<.+?>','',string)
    return dst
...
def main():
    ...
    for line in text:
        ...
        val1 = remove_inner_link(val1)
        val1 = remove_outer_link(val1)
        val1 = remove_htmltag(val1)
...
# [[File:ファイル名|thumb|ファイルの大きさ|説明]]c